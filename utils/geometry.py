...
"""
Geometry utilities: centerline, corridor, position classification — FIXED

Key fix: Corridor fallback is quieter (only logged once per call).
"""
import numpy as np
from typing import Optional, Callable, Tuple
from config import COL_LEFT_END, COL_RIGHT_START
...
def get_hpos_x(det, mask: Optional[np.ndarray], img_w: int) -> float:
    """Return x-coordinate for horizontal position classification."""
    if mask is not None:
        cols = np.where(mask.any(axis=0))[0]
        if len(cols) > 0:
            return float(cols.mean())
...
def extract_sidewalk_centerline(
    ...
    """Extract sidewalk centerline via per-row midpoint regression."""
    if sidewalk_mask is None:
        return None, 0, None
...
def compute_corridor_bounds(
    ...
    if centerline_fn is None or path_width_px < 30:
        return _default
    ...
    if not (0 <= cl_x <= img_w):
        print(f"  [Corridor] Invalid centerline (x={cl_x:.0f}) — using default split")
        return _default
    ...
    if (right_edge - left_edge) < img_w * 0.20:
        print("  [Corridor] Too narrow — using default split")
        return _default
    ...
    print(f"  [Corridor] y={y_eval}: x={cl_x:.0f}, "
          f"width={path_width_px}px → {left_edge}–{right_edge}")
    return bounds
