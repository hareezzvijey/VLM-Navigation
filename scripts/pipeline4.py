"""
VLM Pipeline v6 — Sidewalk-Aware Navigation
=============================================
Built on v5. New upgrade tagged [SW]:
...
# Numeric scores for state fusion
_STATE_SCORE = {"walkable": 0, "crowded": 1, "blocked": 2, "unknown": 0}
_SCORE_STATE = {0: "walkable", 1: "crowded", 2: "blocked"}
...
def analyse_sidewalk_local(
    ...
    for col, (cx1, cx2) in col_bounds.items():
        ...
        # Accumulate obstacle pixel area inside this column, on-sidewalk only
        hard_obs_px = 0
        soft_obs_px = 0
        ...
        for det in detections:
            ...
            # Intersection of object box with this column strip
            ...
            obj_col_px = (ox2 - ox1) * (oy2 - oy1)
            if role in HARD_BLOCK_ROLES:
                hard_obs_px += obj_col_px
            elif role in SOFT_BLOCK_ROLES:
                soft_obs_px += obj_col_px

        hard_coverage = hard_obs_px / sw_col_px
        soft_coverage = soft_obs_px / sw_col_px

        if hard_coverage >= SW_BLOCK_COVERAGE:
            result[col] = "blocked"
        elif hard_coverage >= SW_CROWD_COVERAGE or soft_coverage >= SW_CROWD_COVERAGE:
            result[col] = "crowded"
        else:
            result[col] = "walkable"
...
def _fuse_states(global_st: str, local_st: str) -> str:
    """
    [SW] Weighted fusion of global and sidewalk-local column states.

    Numeric mapping: walkable=0, crowded=1, blocked=2.
    Weighted average rounded to nearest integer → back to state string.
    "unknown" from local (no sidewalk in column) → defer to global entirely.
    """
    if local_st == "unknown":
        return global_st   # no local data, trust global

    g = _STATE_SCORE.get(global_st, 0)
    l = _STATE_SCORE.get(local_st,  0)
    fused = SW_LOCAL_WEIGHT * l + SW_GLOBAL_WEIGHT * g
    return _SCORE_STATE[round(fused)]
...
def analyse_free_space(
    ...
    for col in col_bounds:
        ...
        if col in hard_blocked_cols:
            ...
            # [FIX2] No obstacle, no confirmed surface → assume walkable
            global_result[col] = "walkable"
...
class VLMPipeline:
    ...
    def visualize(self, results: dict, save_name: str = "output_v5.png"):
        ...
        axes[1].imshow(image);            axes[1].set_title("Navigation v6 (sidewalk-aware)"); axes[1].axis("off")
        plt.tight_layout()
...
def test():
    parser = argparse.ArgumentParser(description="VLM Navigation Pipeline v6 — Sidewalk-Aware")
    ...
    parser.add_argument("--output",   default="output_v6.png")
