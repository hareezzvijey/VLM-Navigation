...
"""
LLM Client — FIXED: better post-processing, complete sentences

Key fixes:
1. _post_process no longer cuts off sentences mid-way
2. Retry logic improved
3. Grok/Gemini/Ollama/OpenAI/Transformers all supported
"""
import os
import time
from typing import Optional
...
class LLMClient:
    ...
    def load(self) -> bool:
        ...
        try:
            if self.provider == "grok":
                api_key = self.api_key or os.environ.get("XAI_API_KEY")
                if not api_key:
                    print("[LLM] No Grok API key. Set XAI_API_KEY or --llm-api-key")
                    return False
                self.client  = {"base_url": "https://api.x.ai/v1", "api_key": api_key}
                self._loaded = True
                print(f"[LLM] Grok {self.model} loaded ✓")
                return True
                ...
                api_key = self.api_key or os.environ.get("GEMINI_API_KEY")
                if not api_key:
                    print("[LLM] No Gemini API key. Set GEMINI_API_KEY")
                    return False
                genai.configure(api_key=api_key)
                self.client  = genai.GenerativeModel(self.model)
                self._loaded = True
            ...
            try:
                if self.provider == "grok":
                    ...
                    if r.status_code == 200:
                        ...
                        return self._post_process(text)
                    print(f"[LLM] Grok error: {r.status_code} - {r.text[:200]}")
                    return None
    ...
    def _post_process(self, text: str) -> str:
        """
        FIXED: Clean output without cutting mid-sentence.
        - Remove prompt echo
        - Keep ONE complete sentence
        - Remove newlines
        """
        if not text:
            return ""
        ...
        # Remove prompt leakage markers
        for marker in ["OUTPUT:", "OUTPUT :", "Response:", "Answer:", "RESPONSE:"]:
            if marker in text:
                text = text.split(marker)[-1].strip()

        # FIXED: Extract first COMPLETE sentence (ending with period)
        # Don't just split on first dot — that cuts "Move forward." to "Move forward"
        sentences = [s.strip() for s in text.split(".") if s.strip()]
        if sentences:
            # Reconstruct first meaningful sentence
            first = sentences[0]
